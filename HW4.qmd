...
#### 1. Select suitable environmental conditions for oysters

```{r}
#| code-fold: true
# Set workflow for selecting suitable environment conditions for oysters

oysters <- function(Species, SST, Depth, EEZ, SST_extent){
  ...
  oyster_map <- tm_shape(background_data) + 
                  ...
                  tm_raster(palette = "red",
                            title = "",
                            labels = paste("Suitable locations for", Species)) +
                ...
                tm_compass(position = c(0.05, 0.8),
                           size = 1.3)
  map_table <- list(oysterMap = oyster_map, oysterArea_Table = suitable_area_tab)
  return(map_table)
}
```

#### 2. Select suitable environmental conditions for Dungeness Crab

```{r}
#| code-fold: true
# Set workflow for selecting suitable environment conditions for crab

crabs <- function(Species, SST, Depth, EEZ, SST_extent){
  # Drop the units (m) of Bathymetry data
  Depth <- drop_units(Depth)
  ...
  EEZ_rasterized <- terra::rasterize(wc_region_terra,
                                     crab_suit_terra,
                                     field = "rgn_id") 
  EEZ_mask <- mask(EEZ_rasterized, crab_suit_EEZ)
  ...
  suitable_area_tab <- kable(
    suitable_area, 
    caption = "Table 2 Suitable Area for Dungeness Crab in Each Exclusive Economic Zones (km²)",
    col.names = c("Name", "Area (km²)")) 
  
  
  
  # Map the suitable locations for crab
  ## Download base map from OSM of the bounding box
  background_data <- tmaptools::read_osm(sf::st_bbox(SST_extent))
  ...
  ## Map
  crab_map <- tm_shape(background_data) + 
                  tm_rgb() +
                  tm_shape(crab_suit_sameCRS) +
                  tm_raster(palette = "purple",
                            title = "",
                            labels = paste("Suitable locations\nfor", Species)) +
                  tm_credits("© OpenStreetMap contributors",
                             position=c("left", "bottom")) +
                  tm_layout(legend.text.size = 0.9,
                            legend.text.fontface = "bold",
                ...
                tm_compass(position = c(0.05, 0.8),
                           size = 1.3)
  map_table <- list(crabMap = crab_map, crabArea_Table = suitable_area_tab)
  return(map_table)
}
```
...
```{r}
oysters <- oysters("oysters", SST = SST_aveC, Depth = Bathymetry_resampled, EEZ = wc_region, SST_extent = SST_extent)

crabs <- crabs("Dungeness Crab", SST = SST_aveC, Depth = Bathymetry_resampled, EEZ = wc_region, SST_extent = SST_extent)

tmap_arrange(oysters$oysterMap, crabs$crabMap, ncol = 2)
```

Figure 1 Suitable locations for oysters (left) and Dungeness Crab

```{r}
oysters$oysterArea_Table
crabs$crabArea_Table
```

According to Figure 1, we can see that both oysters and Dungeness Crab have wide distribution across the west coast of the US. Particularly, oysters are more concentrated in the southern part, but Dungeness Crab prefers the northern part. The statistic of suitable areas within each Exclusive Economic Zone shows similar patterns, where Oregon and Washington are more suitable for Dungeness Crab aquaculture, while Central and Southern California are more suitable for oysters aquaculture (Table 1 and Table 2).
